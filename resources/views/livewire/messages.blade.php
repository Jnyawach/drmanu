                    ...
                    <table class="table contacts" >
                        ...
                        @foreach($messages as $message)
                                ...
                                <tr class="unread">
                                    ...
                                    <td><a href="{{route('messages.show',$message->id)}}" title="read">
                                            <i class="fas fa-bookmark ms-2"></i></a></td>
                                    <td>
                                        <button type="submit" wire:click="deleteMessage({{$message->id}})" class="btn-link text-danger"><i class="far fa-trash-alt"></i></button>
                                    </td>
                                    <td>
                                        <button type="submit" class="btn-link {{$message->status==1?'text-success':''}}" title="Mark as Read" wire:click="readMessage({{$message->id}})">
